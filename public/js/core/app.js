    ...
    if (e.keyCode !== 38 && e.keyCode !== 40 && e.keyCode !== 13) {
      ...
      if (value != '') {
        ...
        var $startList = '',
          ...
          $htmlList = '',
          $bookmarkhtmlList = '',
          $pageList = '<li class="d-flex align-items-center">' + "<a href=\"".concat(assetPath, "app/inquiries/list\">") + '<h6 class="section-label mt-75 mb-0">Enquiries</h6>' + '</a>' + '</li>',
          $activeItemClass = '',
          $bookmarkIcon = '',
          $defaultList = '',
          a = 0;
        ...
        $.ajax({
          url: "".concat(assetPath, "app/inquiries/search"),
          ...
          success: function success(data) {
            ...
            if (data.listItems.length < 1) {
              $startList = '<li class="auto-suggestion"' + '<div class="d-flex align-items-center justify-content-between w-100" href="#">' + '<span>No Enquiries found</span>' + '</div>' + '</li>';
            }
            for (var i = 0; i < data.listItems.length; i++) {
              console.log(data.listItems[i].name, 'naaame');
              if (a === 0) {
                ;
                $activeItemClass = 'current_item';
              } else {
            ...
            $htmlList = $pageList.concat($startList);
            $('ul.search-list').html($htmlList);
            // concatinating otherlist with startlist
            // $bookmarkhtmlList = $startList.concat($otherList);
            // $('ul.search-list-bookmark').html($bookmarkhtmlList);
            console.log($startList, 'start list');
          },
